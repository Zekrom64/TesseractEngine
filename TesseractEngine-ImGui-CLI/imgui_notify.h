...
constexpr auto NOTIFY_MAX_MSG_LENGTH = 4096;		// Max message content length
constexpr auto NOTIFY_PADDING_X = 20.f;				// Bottom-left X padding
constexpr auto NOTIFY_PADDING_Y = 20.f;				// Bottom-left Y padding
constexpr auto NOTIFY_PADDING_MESSAGE_Y = 10.f;		// Padding Y between each message
constexpr uint64_t NOTIFY_FADE_IN_OUT_TIME = 150;	// Fade in and out duration
constexpr uint64_t NOTIFY_DEFAULT_DISMISS = 3000;	// Auto dismiss after X ms (default, applied only of no data provided in constructors)
constexpr auto NOTIFY_OPACITY = 1.0f;				// 0-1 Toast opacity
constexpr auto NOTIFY_TOAST_FLAGS = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoFocusOnAppearing;
...
public:
	...
	NOTIFY_INLINE void set_dismiss_time(int dismissMsec) { this->dismiss_time = dismissMsec; }
...
public:
	...
	NOTIFY_INLINE void reset_creation_time() {
...
namespace ImGui
{
	...
	NOTIFY_INLINE void InsertNotification(const ImGuiToast& toast)
	{
		...
		notifications.rbegin()->reset_creation_time();
	}
